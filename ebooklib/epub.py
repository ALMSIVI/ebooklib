...
from ebooklib.utils import parse_string, parse_html_string, guess_type, get_pages_for_items, relpath
            ...
            for item in items:
                if isinstance(item, tuple) or isinstance(item, list):
                    ...
                    if isinstance(item[0], EpubHtml):
                        a = etree.SubElement(li, 'a', {'href': relpath(item[0].file_name, nav_dir_name)})
                    elif isinstance(item[0], Section) and item[0].href != '':
                        a = etree.SubElement(li, 'a', {'href': relpath(item[0].href, nav_dir_name)})
                    ...
                    li = etree.SubElement(ol, 'li')
                    a = etree.SubElement(li, 'a', {'href': relpath(item.href, nav_dir_name)})
                    a.text = item.title
                elif isinstance(item, EpubHtml):
                    li = etree.SubElement(ol, 'li')
                    a = etree.SubElement(li, 'a', {'href': relpath(item.file_name, nav_dir_name)})
                    a.text = item.title
        ...
        if len(self.book.guide) > 0 and self.options.get('epub3_landmark'):
            ...
            for elem in self.book.guide:
                ...
                a_item = etree.SubElement(li_item, 'a', {
                    ...
                    'href': relpath(_href, nav_dir_name)
                })
                a_item.text = _title
